...
impl BaseClient {
    ...
    pub async fn send_erc20(&self, private_key: &str, token_address: &str, to: &str, amount: &str) -> Result<String, ZetaError> {
        ...
        let call = contract.transfer(to_addr, amount_wei.into());
    ...
    pub async fn sign_message(private_key: &str, message: &str) -> Result<String, ZetaError> {
        let wallet: LocalWallet = private_key.parse::<LocalWallet>()
            .map_err(|e| ZetaError::Internal(format!("Invalid private key: {}", e)))?;

        let signature = wallet.sign_message(message).await
            .map_err(|e| ZetaError::Internal(format!("Signing failed: {}", e)))?;

        Ok(format!("{}", signature))
    }

    pub fn verify_signature(message: &str, signature: &str, address: &str) -> Result<bool, ZetaError> {
        let sig = Signature::from_str(signature)
            .map_err(|e| ZetaError::Internal(format!("Invalid signature format: {}", e)))?;

        let recovered = sig.recover(message)
            .map_err(|e| ZetaError::Internal(format!("Recovery failed: {}", e)))?;
